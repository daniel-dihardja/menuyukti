...
- **Suggested checks before a PR:** from the repo root run `pnpm format-check`, `pnpm check-types`, `pnpm lint`; from `apps/graphql` run `make lint`, `make typecheck`, `make test`; from `apps/agents` run `make lint`, `make typecheck`, `make test`; from `apps/gentic-agents` and `packages/gentic` run `go test ./...` and `make lint` (or rely on CI for golangci-lint).
...
- App READMEs: `apps/agents/README.md`, `apps/gentic-agents/README.md`, `apps/graphql/README.md`, `apps/web/README.md`
