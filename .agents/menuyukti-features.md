...
| style pack, visual style, Instagram style, style spec                     | `apps/graphql/data_sources/models/visual_style.py`, `apps/web/app/api/styles/`, `apps/web/lib/styles/style-spec.ts`, `apps/agents/agents/core/style_spec/`, `apps/web/lib/posts/build-instagram-post-prompt.ts`, `apps/web/app/api/posts/generate/route.ts`, `apps/web/app/(protected)/ig-studio/styles/` | `VisualStyle`, `styleSpec`, `StyleSpec`, `compileStyleSpec`, `parseStyleControlOverrides`, `draft_style_spec_from_image`, `styleId` on generate            | Workspace-scoped reusable look (name + rules and/or Style Spec v1 + one media reference image), with `created_by_clerk_user_id`. Managed in IG Studio Styles; selected in Post Creator and compiled into Leonardo prompts via `styleId`.                                                                                                                                                                                                         |
...
- `apps/web/app/(protected)/ig-studio/styles/` (library, create, edit)
- `apps/web/app/api/styles/draft-from-image/route.ts` (BFF → agents vision draft)
