...
| style pack, visual style, location style, Instagram style, style spec     | `apps/graphql/data_sources/models/location_style.py`, `apps/web/app/api/location-styles/`, `apps/web/lib/location-styles/style-spec.ts`, `apps/agents/agents/core/style_spec/`, `apps/web/lib/posts/build-instagram-post-prompt.ts`, `apps/web/app/api/posts/generate/route.ts` | `LocationStyle`, `styleSpec`, `StyleSpec`, `compileStyleSpec`, `parseStyleControlOverrides`, `draft_style_spec_from_image`, `styleId` on generate          | Location-scoped reusable look (name + rules and/or Style Spec v1 + one media reference image). Managed on the location detail page (manual or create-from-image); selected in Post Creator and compiled into Leonardo prompts via `styleId`.                                                                                                                                                                                                     |
...
- `apps/web/app/(protected)/analytics/locations/location-styles-section.tsx` (CRUD + create-from-image on location detail)
- `apps/web/app/api/location-styles/draft-from-image/route.ts` (BFF → agents vision draft)
