...
- **create_location_profile** — Generate and persist a **new** location profile from venue and operating data. Always use this when the user asks to create, generate, or build a location profile. It always writes a fresh profile (overwriting any prior one for this run); do not call **fetch_location_profile** first for creation-only requests.

Workflow:
1. When the user asks to **create** or **generate** a location profile: call **create_location_profile** once and respond with a short confirmation.
	...
	actor := react.NewReactActor(
		...
		react.WithTools(
			...
			react.NewToolWithState(
				...
				"Generates and saves a new location profile from venue and operating data for the current location_id and analytics_id. Always creates a fresh profile; does not reuse an existing saved profile.",
