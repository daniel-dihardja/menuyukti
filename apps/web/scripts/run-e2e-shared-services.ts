...
type ServiceName = "web" | "analytics";
...
const SERVICES_BY_SUITE: Record<string, ServiceName[]> = {
  ...
  "test:e2e:api:contracts": ["web", "analytics"],
  "test:e2e:release-gate": ["web", "analytics"],
};
...
function getSuitesFromInput(): string[] {
  ...
  const fromEnv = process.env.E2E_SUITE_LIST?.split(",")
    .map((item) => item.trim())
    .filter(Boolean);
...
function requiredServicesForSuites(suites: string[]): ServiceName[] {
  ...
  for (const suite of suites) {
    const deps = SERVICES_BY_SUITE[suite] ?? ["web", "analytics"];
...
async function run() {
  ...
  console.log(
    `[e2e:shared] required services: ${required.join(", ") || "none"}`,
  );
  ...
  try {
    ...
    for (const service of started.reverse()) {
      ...
      console.log(
        `[e2e:shared] stopped ${service.name} (log: ${service.logPath})`,
      );
    }
  }
