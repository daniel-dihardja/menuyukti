...
| `getCachedWorkflowsByLocation(userId, locationId)`                   | `graphqlWorkflowsByLocationCacheTag(userId, locationId)`          | `app/api/workflows/create/route.ts`, `app/api/workflows/[id]/route.ts` PATCH/DELETE (via `revalidateLocationScopedLists`)                           |
