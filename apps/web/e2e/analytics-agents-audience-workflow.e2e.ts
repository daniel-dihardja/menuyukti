...
async function run() {
  ...
  const route = `${baseUrl}/agents/audience`;
  await page.goto(route, { waitUntil: "domcontentloaded" });
  await page.getByRole("heading", { name: /audience/i }).first().waitFor({
    state: "visible",
    timeout: 30_000,
  });
  ...
  const runButton = page.locator('button[aria-controls="audience-agent-output"]').first();
  await runButton.waitFor({ state: "visible", timeout: 10_000 });
  const postResponsePromise = page.waitForResponse(
    (response) =>
      response.url().includes("/api/agents/audience") &&
  ...
  const outputRegion = page.locator("#audience-agent-output");
  await outputRegion.waitFor({ state: "visible", timeout: 10_000 });

  if (postResponse.status() === 200) {
    await outputRegion.getByText(/top items/i).first().waitFor({
      state: "visible",
      timeout: 20_000,
    });
    ...
    throw new Error(`Audience run returned unexpected status ${postResponse.status()}: ${body}`);
  }

  const screenshotPath = path.join(artifactsDir, "analytics-agents-audience-final.png");
  ...
  console.log(`[e2e] agents-audience-route: ${route}`);
  console.log(`[e2e] agents-audience-status: ${postResponse.status()}`);
