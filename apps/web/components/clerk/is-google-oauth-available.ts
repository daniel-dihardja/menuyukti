 ...
 */
export function isGoogleOauthAvailable(clerk: ClerkWithOauthEnvironment | null | undefined): boolean {
  if (!clerk?.loaded) {
    return false
  }
  const strategies = clerk.__internal_environment?.userSettings?.authenticatableSocialStrategies ?? []
