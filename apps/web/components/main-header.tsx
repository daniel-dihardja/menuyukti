...
export function MainHeader() {
  ...
  /** Guest menu keeps brand chrome; skip the mobile sheet (Print Shop / Studio copy). */
  const showMobileNavSheet = !isPublicLocation
    ...
    >
      <div className="box-border flex h-14 w-full min-w-0 items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex shrink-0 items-center gap-2 pe-3 sm:pe-4">
          ...
          <Show when="signed-out">
            {showProductNav || isPublicLocation ? (
              <span className={cn(isPublicLocation ? 'inline-flex' : 'hidden sm:inline-flex')}>
