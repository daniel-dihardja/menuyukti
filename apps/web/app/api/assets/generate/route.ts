...
export async function POST(req: Request) {
  ...
  const sourceKey = userObjectKey(userId, name)
  ...
  try {
    ...
    console.error('[assets/generate] S3 GetObject failed', {
      userIdPrefix: userId.slice(0, 8),
      message: err instanceof Error ? err.message : String(err),
      sourceName: name,
    })
    return NextResponse.json({ message: 'Failed to read source asset' }, { status: 502 })
  }
  ...
  try {
    ...
    console.error('[assets/generate] image preprocessing failed', {
      userIdPrefix: userId.slice(0, 8),
      message: err instanceof Error ? err.message : String(err),
      sourceName: name,
    })
  ...
  try {
    ...
    console.error('[assets/generate] leonardo processing failed', {
      flow,
      userIdPrefix: userId.slice(0, 8),
  ...
  try {
    ...
    console.error('[assets/generate] S3 PutObject failed', {
      userIdPrefix: userId.slice(0, 8),
      message: err instanceof Error ? err.message : String(err),
      sourceName: name,
    })
