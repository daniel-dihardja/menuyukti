...
export async function GET(req: Request) {
  ...
  if (!name || !isSafeReelFilename(name)) {
    return NextResponse.json({ message: 'Invalid filename' }, { status: 400 })
  }

  const contentType = reelContentTypeForFilename(name)
  if (!contentType) {
    return NextResponse.json({ message: 'Invalid filename' }, { status: 400 })
  }

  const key = await resolveObjectKey(mediaAccess.access, 'reels', name)
  if (!key) {
  ...
  try {
    ...
    console.error('[reels/download] S3 GetObject failed', {
