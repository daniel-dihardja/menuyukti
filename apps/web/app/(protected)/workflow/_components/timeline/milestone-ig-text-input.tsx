...
import type { IgFormatEntry, IgFormatType } from '@/lib/graphql/node-schemas'
import { resolveIgFormatEntriesForText } from '@/lib/milestones/ig-text-input'

import { MilestoneFieldDescription } from './milestone-field-description'
import { useTimelineWorkspaceState } from '../timeline-context'

export type MilestoneIgTextInputProps = {
  milestoneId: string
  notes: string
...
function weekdayLabel(day: IgFormatEntry['day']): string {
...
export function MilestoneIgTextInput({
  milestoneId,
  notes,
  onNotesChange,
  onNotesBlur,
  ...
  return (
    <FieldGroup className="gap-4">
      {entries.length === 0 ? (
        <Alert>
          <AlertDescription>
            <p className="font-medium">{t('milestoneIgTextInputNoPriorFormatTitle')}</p>
            <p className="text-muted-foreground">{t('milestoneIgTextInputNoPriorFormatBody')}</p>
          </AlertDescription>
        </Alert>
      ) : (
        <FieldSet>
          <FieldLegend>{t('milestoneIgTextInputEntriesLabel')}</FieldLegend>
          <MilestoneFieldDescription content={t('milestoneIgTextInputEntriesDescription')} />
      ...
      <Field>
        <FieldLabel htmlFor={`ig-text-notes-${milestoneId}`}>
          {t('milestoneIgTextInputNotesLabel')}
        </FieldLabel>
        <MilestoneFieldDescription content={t('milestoneIgTextInputNotesDescription')} />
        <Textarea
          className="min-h-[120px] resize-y whitespace-pre-wrap"
          disabled={disabled}
          id={`ig-text-notes-${milestoneId}`}
          onBlur={onNotesBlur}
          onFocus={onNotesFocus}
          onChange={(e) => onNotesChange(e.target.value)}
          onClick={(e) => e.stopPropagation()}
          onPointerDown={(e) => e.stopPropagation()}
          placeholder={t('milestoneIgTextInputNotesPlaceholder')}
          value={notes}
        />
      </Field>
    </FieldGroup>
