...
import { HeatmapMatrixCard, HeatmapMatrixEmbedded } from './heatmap-matrix'
...
export type MenuHeatmapContentProps = {
  ...
  /** Controlled category filter (e.g. from nuqs on the analytics report page). */
...
export function MenuHeatmapContent({
  menuHeatmaps,
  matrixItems,
  locale,
  dailyStartHour,
  ...
  return (
    <div className="flex flex-col gap-4">
      <Field className={cn('gap-2', isCompact ? 'max-w-full' : 'max-w-sm')}>
        <FieldLabel htmlFor={categoryFilterId}>{t('filters.categoryLabel')}</FieldLabel>
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              {CATEGORY_ORDER.map((category) => {
                const checkboxId = `${categoryFilterId}-matrix-${category}`
                return (
      ...
      {matrixFilterEmpty || filteredEmpty ? (
        ...
        <Tabs value={view} onValueChange={(v) => setView(v as 'daily' | 'weekly')}>
          ...
          <TabsContent value="daily" className="mt-4 flex flex-col gap-4">
            {dailyEmpty ? (
              <p className="text-muted-foreground text-sm">{t('emptyDaily')}</p>
            ) : (
              <>
                {!isCompact ? (
                  <InsightStrip
          ...
          <TabsContent value="weekly" className="mt-4 flex flex-col gap-4">
            {weeklyEmpty ? (
              <p className="text-muted-foreground text-sm">{t('emptyWeekly')}</p>
            ) : (
              <>
                {!isCompact ? (
                  <InsightStrip
