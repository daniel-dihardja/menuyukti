...
function normalizeTheme(raw: unknown): 'holiday' | 'promotion' | 'engagement' {
...
export function parsePostScheduleJson(postScheduleJson: string | null | undefined): PostSlot[] {
  ...
  try {
    ...
    return parsed.map((row): PostSlot => {
      ...
      return {
        ...
        scheduled_time: slot.scheduled_time != null ? String(slot.scheduled_time) : undefined,
        theme: normalizeTheme(slot.theme),
        format: normalizeFormat(slot.format),
        focus_item: slot.focus_item != null ? String(slot.focus_item) : null,
        carousel_items: Array.isArray(slot.carousel_items) ? slot.carousel_items.map(String) : null,
        carousel_narrative:
          slot.carousel_narrative != null ? String(slot.carousel_narrative) : null,
