...
export default async function Page({ params }: PageProps) {
  ...
  try {
    ;[campaignData, briefData] = await Promise.all([
      graphqlQuery<CampaignDetailData>(CAMPAIGN_DETAIL_QUERY, { id: campaignIdStr }, userId),
      graphqlQuery<CampaignBriefData>(CAMPAIGN_BRIEF_QUERY, { campaignId: campaignIdStr }, userId),
    ])
  } catch {
    notFound()
  }
  ...
  if (briefAnalyticsId != null) {
    ...
    locationSummary = rawSummary && rawSummary.length > 0 ? rawSummary : null
