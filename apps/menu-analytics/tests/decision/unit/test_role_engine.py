...
def test_assign_roles_growth_lever_and_anchor():
    ...
    roles = assign_roles(item)

    assert RoleType.PROFIT_ANCHOR in roles
    assert RoleType.GROWTH_LEVER in roles
    assert RoleType.VELOCITY_DRIVER in roles
    assert RoleType.TRAFFIC_DRIVER not in roles
    assert RoleType.DRAG not in roles
