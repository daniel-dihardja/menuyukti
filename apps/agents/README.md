...
- **Domain** (`agents/domain/skill_runner/`): skill-driven prepare flows. **`POST /milestones/{id}/prepare`** loads runtime `SKILL.md` from the **`agent-skills`** package (e.g. `location_profile`, `instagram_campaign_schedule` via `data_task` in the JSON body; default `location_profile`). Prefetch + LLM instructions live in that package; saving to `milestonedata` is handled by core `milestone_data`, not declared in the skill file. The format endpoint is **not** domain-specific; milestone UI is one client.
